...
class AdamHDOptimizer(tf.train.GradientDescentOptimizer):
    ...
    def minimize(self, loss, global_step):
        ...
        # Create and retrieve slot variables for direstion of steps and current learning_rate alpha
        ...
        #calculate bais corrected variance estimate
        with tf.control_dependencies(v_updates+m_updates+mu_power_update+[alpha_update]):
            ...
            snrs = [tf.divide(m, tf.sqrt(r) + self._eps) for m, r in zip(ms_hat, rs)] #signal to noise ratio
